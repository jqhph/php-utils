...
class Connection implements ConnectionInterface
{
    ...
    {
        return function (array $request, self $connection, Transport $transport = null, $options) use ($handler, $logger, $tracer) {
            $this->lastRequest = [];
            $this->lastRequest['request'] = $request;

            // Send the request using the wrapped handler.
            $response = Core::proxy($handler($request), function ($responses) use ($connection, $transport, $logger, $tracer, $request, $options) {
                /* @var ResponseInterface $response */
                $response = $responses[0];

                $body = $response->getBody()->getContents();
                $totalTime = $responses['total_time'];
                $fullPath = $request['headers']['Host'][0].$request['uri'];
                $statusCode = $response->getStatusCode();

                $this->lastRequest['response'] = $responses;

                if ($statusCode == 500) {
                    if ($statusCode == 500 && ! $body) {
                        $this->notActiveNode($request, $transport, $connection, $options, $responses);
                    } else {
                        // Something went seriously wrong, bail
                        $exception = new TransportException($response->getReasonPhrase());
                        $this->logRequestFail(
                            $request['http_method'],
                            $fullPath,
                            $request['body'],
                    ...
                    if ($statusCode >= 400 && $statusCode < 500) {
                        ...
                        $this->process4xxError($request, $response, $ignore, $totalTime, $fullPath);
                    } elseif ($statusCode >= 500) {
                        $ignore = isset($request['client']['ignore']) ? $request['client']['ignore'] : [];
                        $this->process5xxError($request, $response, $ignore, $totalTime, $fullPath);
                    }
                ...
                return isset($request['client']['verbose']) && $request['client']['verbose'] === true ? $response : $body;
            });
    ...
    protected function notActiveNode(array $request, Transport $transport, ConnectionInterface $connection, $options, array $responses)
    {
        $response = $responses[0];
     ...
     */
    private function process4xxError($request, ResponseInterface $response, $ignore, $totalTime, $fullPath)
    {
        $statusCode = $response->getStatusCode();
     ...
     */
    private function process5xxError($request, ResponseInterface $response, $ignore, $totalTime, $fullPath)
    {
        $statusCode = $response->getStatusCode();
        $responseBody = $response->getBody()->getContents();
    ...
    private function tryDeserializeError(ResponseInterface $response, $errorClass)
    {
        $body = $response->getBody()->getContents();
        $statusCode = $response->getStatusCode();
        $error = $this->serializer->deserialize($body, []);
        if (is_array($error) === true) {
            ...
            if (isset($error['error']['reason']) === true) {
                ...
                $original = new $errorClass($body, $statusCode);

                return new $errorClass("$type: $cause", $statusCode, $original);
            } elseif (isset($error['error']) === true) {
                // <2.0 semi-structured exceptions
                $original = new $errorClass($body, $statusCode);

                return new $errorClass($error['error'], $statusCode, $original);
            }

            // <2.0 "i just blew up" nonstructured exception
            // $error is an array but we don't know the format, reuse the response body instead
            return new $errorClass($body, $statusCode);
        }
